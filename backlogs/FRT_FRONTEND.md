...
| [x]    | P0       | `TASK-FRT-180` | `TASK-FRT-175` | Visible analysis progress: simplify orphaned `AnalysisProgressTracker` to 4 user-facing stages, mount on Analysis + Documents; convert `useProjectDocuments` to react-query with polling while docs in-flight; delete the raw-SSE `<Link>` dead end. `[x] Implemented (Visible Analysis Progress + Document Polling)` | `Fable5 L1 Prompt — PATCH 6` |
| [x]    | P0       | `TASK-FRT-181` | None       | Fix "Retry processing": raw `fetch` sends no Authorization (guaranteed 401, silent failure) → use `apiClient.post` + visible error toast. `[x] Implemented (Authenticated Retry + Error Toast)` | `Fable5 L1 Prompt — PATCH 7` |
