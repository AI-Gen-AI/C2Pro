...
| [x] | P2 | `TASK-BCK-091` | None | Backfill Supabase CLI migration mirror to match Alembic schema. The Supabase CLI mirror in `supabase/migrations/` is missing tables that exist only in Alembic (confirmed: `procurement_bom_items`). Root cause discovered via PR #183's `20260708000100_add_waitlist_signups.sql` which tries to `ALTER TABLE public.procurement_bom_items ADD COLUMN ... source_document_id` but the table was never created in the mirror. ✅ Completed 2026-07-09: Audited Alembic migrations vs Supabase mirror, identified 33 missing tables, and backfilled them in 15 SQL migration files with fail-closed RLS and indices. MASTER review replayed all 20 files against a fresh Postgres+pgvector container (matching Supabase Preview CI) and found the first delivery didn't actually apply: one missed Alembic migration (`approvalstatus` enum) and 6 in-file FK forward-references. Fixed (missing enum + 6 trailing `ALTER TABLE ADD CONSTRAINT`s); full 20-file replay now succeeds end-to-end; see `C2PRO_MASTER_BACKLOG.md` Change Log for full detail. | PR #183 discovery |
