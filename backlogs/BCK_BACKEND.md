...
| [ ] | P2 | `TASK-BCK-091` | None | Backfill Supabase CLI migration mirror to match Alembic schema. The Supabase CLI mirror in `supabase/migrations/` is missing tables that exist only in Alembic (confirmed: `procurement_bom_items`). Root cause discovered via PR #183's `20260708000100_add_waitlist_signups.sql` which tries to `ALTER TABLE public.procurement_bom_items ADD COLUMN ... source_document_id` but the table was never created in the mirror — only in Alembic migrations. Need to audit all Alembic migrations (`apps/api/alembic/versions/`) vs. `supabase/migrations/` and backfill the missing mirror SQL so Supabase Preview branches build cleanly. Until fixed, `Supabase Preview` CI check fails on any PR that adds new Supabase migrations. | PR #183 discovery |
