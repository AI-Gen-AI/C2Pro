...
| `[x] TASK-V3-013-04` | P0 | 013-02 | Apply `NodeResult` + ban silent `except: return []/None` on material nodes; type channel values; persist errors to events. `[x] Implemented (NodeResult material-node failure envelopes + producer boundary validation)` · Out: trivial passthrough nodes | `analysis/adapters/graph/{schema.py,nodes.py,nodes_extended.py}` | no-silent-failure injection suite | ✅ Done. 2nd pass added NodeResult to N4/N5/N9/N12/N17, N13 DEGRADED, skip-paths. 3rd pass fixed IR-1: N4/N5/N9/N15 validate frozen contracts and store `model_dump(mode="python")` dicts so N8/N11/N17 consumers remain compatible. |
...
| `[x] TASK-V3-013-07` | P0 | 013-04 | Route degraded/failed node counts to a typed Documentation-health signal (consumed by ADR-018). `[x] Implemented (wired documentation-health aggregate)` | `analysis/domain/`, graph state | failed-node→signal unit | ✅ Done. `DocumentationHealthSignal` is built from accumulated `node_results` at N16 final assembly; failed/degraded/skipped counts are available in graph state for ADR-018. |
| `[x] TASK-V3-013-08` | P0 | 013-01 | INV-1 scaffold: `EvidenceRef` + tier enum `{verified,weak,inferred,unverified}` + honest-null helper. `[x] Implemented (INV-1 scaffold)` · Out: full gating (016/018) | `src/evidence/`, `analysis/domain/` | missing-evidence→null+reason unit | ✅ Done: `EvidenceRef`, `EvidenceTier`, and `honest_null()` covered by unit tests. |
...
| `[x] TASK-V3-013-09` | P1 | 013-04 | Remove fabricated minimal EnrichedCoherenceResult fallback in `coherence/graph/graph.py` (~lines 247-258, 297-308): on inner failure return honest null+reason, never synthesize a result (INV-1). `[x] Implemented (honest-null coherence fallback)` Found by Gemini integration review. · Out: scoring logic | `coherence/graph/graph.py` | unit: None inner result → honest failure not fabricated | ✅ Done: result-none sync/async paths return `overall_score=None` + reason without fabricated alerts/signals. |
| `[x] TASK-V3-013-11` | P0 | 013-04 | End-to-end populated-data graph regression test: run compiled graph (ainvoke) with a real tenant + ≥1 risk + ≥1 citation through N8/N11/N17; assert no AttributeError, N8/N17 not FAILED from consumer dict-access, node_results merged. `[x] Implemented (IR-1 populated-data compiled graph regression)` · Out: new features | `tests/analysis/adapters/graph/` | the test itself (RED→GREEN) | ✅ Done. RED reproduced `RiskItem.get` AttributeError in compiled graph; GREEN proves populated risk/citation run completes, N8/N17 are not FAILED, and node_results are deduped. |
