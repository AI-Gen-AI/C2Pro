...
|[ ]|P1|`TASK-OPS-DOCFLOW-015`|`TASK-OPS-DOCFLOW-014`|Fix full backend suite dependency blocker: install or gate the Schemathesis contract suite dependency so `tests/contract/schemathesis/*` can collect in local and CI environments.|`TASK-OPS-DOCFLOW-014 local verification`|
|[ ]|P1|`TASK-OPS-DOCFLOW-016`|`TASK-OPS-DOCFLOW-014`|Fix stale full-suite legacy contract imports for relocated alert and procurement router APIs, including `src.alerts.router` and `get_bom_repository` import paths.|`TASK-OPS-DOCFLOW-014 local verification`|
