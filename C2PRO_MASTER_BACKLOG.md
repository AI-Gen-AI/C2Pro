...
| P0 | `[~] TASK-COH-V1-03` | OpenCode (redispatched) | 01 | Define `LLMRulePort` in domain. Move `AnthropicWrapper` callers to `coherence/adapters/ai/`. Snapshot tests. **Status:** WIP scaffold on `coh-v1/phase-3-opencode` (port + adapter + factory exist; 4 stale `get_anthropic_wrapper()` calls remain in `llm_integration.py` flagged with `TODO(TASK-COH-V1-03 OpenCode redispatch)`; no PHASE-3 report; no snapshot tests; LSP not clean). Acceptance criteria: `blackboard/coh-v1/PHASE-3-opencode-REDISPATCH.md`. |
...
| Date | Milestone |
| ---- | --------- |
| 2026-04-25 | **TASK-COH-V1-03 redispatched to OpenCode** — Original Phase 3 work delivered the `LLMRulePort` Protocol + `LLMRuleResult` dataclass + `LLMRuleEvaluatorAdapter` + `get_llm_rule_evaluator()` factory + port injection in `LlmRuleEvaluator`, but stopped short of: (a) finishing the `CoherenceLLMService` refactor (4 sites still call `get_anthropic_wrapper()` directly despite docstring claiming port delegation — flagged with inline `TODO(TASK-COH-V1-03 OpenCode redispatch)` markers at L283/L362/L443/L685 in `llm_integration.py`), (b) writing the PHASE-3 report file, (c) running snapshot/parity tests (current: `2 failed + 11 passed + 42 errors`), (d) clearing LSP errors. Branch `coh-v1/phase-3-opencode` (commit `2b940597`) carries the WIP + redispatch brief at `blackboard/coh-v1/PHASE-3-opencode-REDISPATCH.md` with full acceptance criteria. |
