...
| Date | Milestone |
| ---- | --------- |
| 2026-04-21 | **EPIC-CORE-DECOUPLE second pass complete** — `[STATUS: SUBTASK-14 DONE - COVERAGE: 92% - PHASES 1-4 COMPLETED]`. Extracted remaining inline AI/business logic from LangGraph nodes into pure domain services + use cases. Phase 1: new `src.analysis.domain.ai_extraction` with `DocumentTypeClassificationService`, `CritiqueExtractionService`, `RaciGenerationService`, `BudgetExtractionService`, `DeterministicRiskRulesService`, `DeterministicWbsRulesService`, and `AIExtractionPort`. Phase 2: new use cases `ClassifyDocumentUseCase`, `CritiqueExtractionUseCase`, `GenerateRaciUseCase`, `ParseBudgetUseCase` (all 100% covered). Phase 3: `router_node`, `critique_node`, `raci_generator_node`, `budget_parser_extended_node` reduced to thin delegate wrappers (nodes.py 87%, nodes_extended.py 85%). Phase 4: `_next_after_critique_v2` now delegates routing to `CritiqueEvaluationService.determine_next_step`. 196 tests GREEN (96 existing + 100 new + 0 regressions). Functional parity preserved (no prompt/LLM config changes). |
