...
| P0 | `TASK-BCK-056` | `TASK-BCK-054,TASK-BCK-055` | Repair duplicate Alembic revisions introduced by overlapping production hotfixes so the live migration chain has one `20260516_0002`, one `20260516_0003`, and one `20260516_0004` head path before deployment. | `PR #133 CI 2026-05-16` |
...
**2026-05-16**: PR #92 follow-ups shipped (branch `chore/pr-92-followups-2026-05-16`). (1) EPIC-DLQ-ADMIN pagination: `GET /admin/dlq` gains `limit` (1–500, default 50) and `offset` (default 0) query params; `DLQListResponse` extended with `total`, `limit`, `offset`, `has_more`; `DLQAdminPort` + `ListDLQEntriesUseCase` updated; `DLQServiceAdminAdapter` uses `.limit().offset()` + a separate `SELECT COUNT(*)`; 6 new boundary-case unit tests + 4 new integration tests. (2) Retry audit log: `POST /admin/dlq/{id}/retry` emits `admin_dlq_retry` structlog event (with `admin_id`, `dlq_id`, `tenant_id`) before the retry call; 2 new integration tests verify the log fires on success and on failure. (3) HITL_APPROVAL_RATE verification: grep confirmed the Gauge was NOT removed in PR #92 and is in active use inside `monitoring.py` (used by `record_hitl_decision` and `record_hitl_approval_rate`); no outside-module production callers; Gauge retained; added 2 regression tests to `test_monitoring_metric_registration.py` guarding against type drift and duplicate re-registration. All 40 tests pass.
