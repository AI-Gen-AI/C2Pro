...
- **Frontend Security (Technical Design v4.0):** RBAC on the frontend is UX ONLY. All authorization MUST be enforced on the backend. Strict Content Security Policy (CSP) must be maintained in `next.config.ts`.
