# Deterministic Rules Engine v0.3 — Final Edition

## Aligned with C2Pro 6 Categories + Golden Dataset Calibration

---

## 1. What Changed from the Previous Iteration

- **Added TECHNICAL and QUALITY categories** — now covers all 6: SCOPE, BUDGET, TIME, TECH, LEGAL, QUALITY
- **Aligned with calibration dataset structure** — evaluators understand `contract`, `budget_items`, `schedule_items`, `bom_items`, and `clauses` from `calibration_dataset.example.json`
- **Added cross-dimensional evaluators** — the tridimensional checks that catch mismatches *between* contract, schedule, and budget (the core C2Pro value proposition)
- **Added golden test cases** for calibration — each evaluator has cases with known `human_score` to tune the impact curves

---

## 2. The 6 Categories with Evaluator Coverage
...
### `rules_engine/deterministic.py` — Complete File (Drop-In Replacement)
...
## 4. Golden Test Cases for Score Calibration

These golden cases use the **calibration_dataset structure** and have known
expected outcomes to validate that the evaluator impact curves produce
realistic scores when fed through the Scoring Arbiter.

```python
"""
tests/coherence/golden_deterministic.py — Golden calibration test cases.

Each case maps a calibration_dataset-style project to expected evaluator
behavior. Used to tune impact curves so the final coherence score
matches human_score within ±5 points.
...
GOLDEN_CASES = [
    {
        ...
        "human_score": 95,
        "clauses": [
            {"id": "c1", "text": "Budget allocated", "data": {
                "current": 100000, "planned": 110000, "currency": "EUR",
            }},
            {"id": "c2", "text": "Schedule on track", "data": {
                "status": "on-track", "end_date": "2030-12-31",
            }},
        ],
        "expected_findings": 0,
    ...
    },
    {
        ...
        "human_score": 82,
        "description": "From calibration_dataset.example.json. Budget items sum to 130k but contract is 120k.",
        "clauses": [
            {"id": "c1", "text": "Contract total 120k", "data": {
                "contract_total": 120000,
                "budget_items": [{"amount": 60000}, {"amount": 70000}],
            }},
            {"id": "c2", "text": "Schedule", "data": {
                "schedule_items": [
                    {"id": "task-1", "name": "Foundations", "start_date": "2030-01-01", "end_date": "2030-01-10", "predecessor_id": None},
                    {"id": "task-2", "name": "Structure", "start_date": "2030-01-05", "end_date": "2030-01-12", "predecessor_id": "task-1"},
                ],
            }},
            {"id": "c3", "text": "BOM", "data": {
                "bom_items": [
                    {"item_name": "Valve X", "required_on_site_date": "2030-02-01", "lead_time_days": 30, "budget_item_id": "budget-1"},
                ],
            }},
        ],
        "expected_rules_triggered": [
            "DET-BUD-SUM",        # 130k > 120k (8.3% deviation)
            "DET-TIM-PREDECESSOR", # task-2 starts before task-1 ends
        ],
    ...
    },
    {
        ...
        "human_score": 42,
        "clauses": [
            {"id": "c1", "text": "Budget way over", "data": {
                "current": 250000, "planned": 150000, "currency": "USD",
            }},
            {"id": "c2", "text": "Schedule delayed", "data": {
                "status": "delayed", "end_date": "2025-06-01",
            }},
            {"id": "c3", "text": "No penalty cap", "data": {
                "penalty": True, "has_penalty_cap": False,
            }},
            {"id": "c4", "text": "Materials", "data": {
                "bom_items": [
                    {"item_name": "Steel beams", "required_on_site_date": "2025-04-01", "lead_time_days": 45},
                    {"item_name": "Concrete mix", "required_on_site_date": "2025-05-01", "lead_time_days": 15},
                ],
            }},
            {"id": "c5", "text": "Quality per industry standards", "data": {}},
        ],
        "expected_rules_triggered": [
            "DET-BUD-OVERRUN",     # 67% over
            "DET-TIM-STATUS",      # delayed
            "DET-TIM-OVERDUE",     # end_date in the past
            "DET-LEG-PENALTY",     # no cap
            "DET-TEC-LEADTIME",    # ordering window closed
            "DET-TEC-BOMBUDGET",   # no budget_item_id
            "DET-QUA-STANDARD",    # vague "industry standards"
        ],
    ...
    },
    {
        ...
        "human_score": 72,
        "clauses": [
            {"id": "c1", "text": "Budget on track", "data": {
                "current": 95000, "planned": 100000, "currency": "EUR",
            }},
            {"id": "c2", "text": "Schedule at risk with long gap", "data": {
                "status": "at_risk",
                "milestones": [
                    {"date": "2030-03-01"},
                    {"date": "2030-09-15"},  # 198-day gap
                ],
            }},
            {"id": "c3", "text": "Contract reviewed recently", "data": {
                "last_review_date": "2026-01-15",
            }},
        ],
        "expected_rules_triggered": [
            "DET-TIM-STATUS",     # at_risk
            "DET-TIM-GAP",        # 198-day milestone gap
        ],
...
```python
"""
tests/coherence/test_golden_deterministic.py — Runs golden cases through the full pipeline.
...
from coherence.rules_engine.deterministic import get_all_deterministic_evaluators
from coherence.scoring import ScoringService
...
class TestGoldenCases:
    ...
    def test_golden_case_score_in_range(self, case, evaluators, scorer):
        """The computed score must fall within the expected range."""
        clauses = [Clause(id=c["id"], text=c["text"], data=c["data"]) for c in case["clauses"]]
        
        all_signals = []
        for clause in clauses:
            for ev in evaluators:
                signal = ev.evaluate_v3(clause)
                if signal:
                    all_signals.append(signal)
        
        score = scorer.calculate_from_signals(
            signals=all_signals,
            num_clauses=len(clauses),
            num_rules=len(evaluators),
        )
        
        lo, hi = case["expected_score_range"]
        assert lo <= score <= hi, (
            f"Case {case['case_id']}: score {score} outside expected "
            f"[{lo}, {hi}] (human_score={case['human_score']}). "
            f"Findings: {len(all_signals)}"
        )

    @pytest.mark.parametrize("case", [c for c in GOLDEN_CASES if "expected_rules_triggered" in c],
                             ids=lambda c: c["case_id"])
    def test_golden_case_expected_rules(self, case, evaluators):
        """The expected rules must fire for each golden case."""
        clauses = [Clause(id=c["id"], text=c["text"], data=c["data"]) for c in case["clauses"]]
        ...
        for clause in clauses:
            for ev in evaluators:
                signal = ev.evaluate_v3(clause)
                if signal:
                    triggered_ids.add(signal.rule_id)
        
        for expected_id in case["expected_rules_triggered"]:
            assert expected_id in triggered_ids, (
                f"Case {case['case_id']}: expected rule {expected_id} did not fire. "
