...
**Status:** Accepted (v2 — locked) · Phase A+B+C shipped · **DEMOTED 2026-08-22** — the Coherence Score is the *relational* Contract-health input, NOT the primary product surface (see 2026-08-22 amendment; the surface is the Health Vector, ADR-018).
...
- **2026-08-22 — DEMOTION (VP product decision; supersedes the "score is the product" framing).** The Coherence Score is **one *relational* dimension** ("do the documents agree?"), NOT the product surface. This is not new: ADR-018 already ruled that *"Coherence is being overloaded to carry a question it cannot answer"* and *"Coherence is demoted to one input of Contract health."* We over-invested in this score (calibration/canary/detection, §G + PRs #532–#552) while the actual surface — the **Health Vector** (ADR-018): per-document decomposition into dimensions with honest-null `missing_data` — stayed an un-wired scaffold. Correction: (1) the Coherence Score feeds the **Contract** dimension of the Health Vector as its `coherence_subscore`; it needs the doc *triplet* to be meaningful and gives a single-document upload almost nothing. (2) A new user's value comes from the **single-document → Health decomposition + gap/`missing_data` alerts** flow (ADR-024), not from this score. (3) All calibration/canary/detection work stays valid as the *backbone of the relational Contract-health input*; nothing is deleted, its **role** is corrected. See ADR-018 (promoted P0-now) and ADR-024 (single-document activation).
