...
|----------|--------|-------|--------|
| P0 | Write unit tests for `_next_after_critique_v2` (5 branch cases) | Backend team | Before Gate 5 |
| P0 | Write unit tests for `core/ai/cost_controller.py` budget enforcement | Backend team | Before Gate 5 |
| P0 | Wire `qa-swarm.yml` into CI to auto-generate tests for P0 gaps | DevOps | Sprint S3 |
| P0 | Fix TD-01: implement `_is_cached_none` in `LLMResultCache` | Backend team | Sprint S3 |
| P1 | Write OCR adapter unit tests with mocked responses | Backend team | Sprint S3 |
| P1 | Write `llm_fallback_client.py` retry/circuit-breaker tests | Backend team | Sprint S3 |

### Strategic Recommendations

1. **Upload coverage.xml in CI**: Modify `tests.yml` to upload `coverage.xml` as an artifact after
   unit tests run. This unblocks the QA swarm's coverage-driven prioritization.

2. **Enforce coverage gate in PR checks**: Add `--cov-fail-under=70` to the unit tests CI step.
   Currently this threshold is configured in `pyproject.toml` but the CI step uses `continue-on-error: true`,
   which means coverage failures do not block merges.

3. **Add red_phase markers for P0 gaps**: New tests for P0 gaps should be marked `@pytest.mark.red_phase`
   initially (TDD discipline), then promoted to `@pytest.mark.unit` once passing.

4. **Resolve the `analysis/adapters/ai/cost_controller.py` shim**: The `*` re-export makes it
   impossible to mock the cost controller in tests targeting the `analysis` module. Migrate to
   explicit import in callers.
