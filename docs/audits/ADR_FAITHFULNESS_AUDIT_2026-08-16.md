...
| **ADR-009** scoring-model unity | One canonical scorer | **Three** coexisting implementations: `scoring.py::ScoringService` (exp-baseline, LIVE), `domain/subscore_calculator.py` (penalty-based, `critical=-30`), `domain/category_state_machine.py` (states). | **MEDIUM** | Converge on one ADR-009-faithful scorer; delete/quarantine the others. |
