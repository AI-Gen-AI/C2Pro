...
import type { StakeholderItem } from "./stakeholderItem";
import type { StakeholdersDataByQuadrant } from "./stakeholdersDataByQuadrant";

export interface StakeholdersData {
  total: number;
  /** Null when not every stakeholder could be loaded, so the count is unknown. */
  key_player_count?: number | null;
  by_quadrant: StakeholdersDataByQuadrant;
  /** True when by_quadrant covers only the stakeholders that could be loaded, not total. */
