...
import { demoDocuments } from "@/lib/demo-data/documents";
...
export default function DocumentsPage() {
  ...
  const documentsByProject = filteredDocuments.reduce(
    (acc, doc) => {
    ...
    {} as Record<
      string,
      {
        projectId: string;
        projectName: string;
        documents: (typeof demoDocuments)[number][];
      }
    >,
  );
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-4">
        ...
        <div className="rounded-lg border bg-card p-4">
          ...
          <p className="text-2xl font-bold text-green-600">
            {demoDocuments.filter((d) => d.status === "Analyzed").length}
          </p>
        </div>
        <div className="rounded-lg border bg-card p-4">
          <p className="text-sm text-muted-foreground">Total Alerts</p>
          <p className="text-2xl font-bold text-amber-600">
            {demoDocuments.reduce((sum, d) => sum + d.alertsFound, 0)}
          </p>
        </div>
        <div className="rounded-lg border bg-card p-4">
          <p className="text-sm text-muted-foreground">Critical Issues</p>
          <p className="text-2xl font-bold text-red-600">
            {demoDocuments.reduce((sum, d) => sum + d.criticalAlerts, 0)}
          </p>
        </div>
      </div>
      ...
      <div className="space-y-4">
        <Accordion
          type="multiple"
          className="space-y-4"
          defaultValue={Object.keys(documentsByProject)}
        >
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div className="text-left">
                    ...
                    <div className="text-sm text-muted-foreground">
                      {projectGroup.projectId} • {projectGroup.documents.length}{" "}
                      documents
                    </div>
                  </div>
                </div>
      ...
      <div className="text-sm text-muted-foreground">
        Showing {filteredDocuments.length} documents across{" "}
        {Object.keys(documentsByProject).length} projects
      </div>
    </div>
  );
}
