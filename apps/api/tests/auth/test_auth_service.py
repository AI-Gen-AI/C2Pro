...
class TestPasswordHashing:
    ...
    def test_hash_password_does_not_emit_bcrypt_backend_warning(self):
        """
        TS-E2E-SEC-TNT-001: Password hashing must work with bcrypt 5 without Passlib warnings.
        """
        project_root = Path(__file__).resolve().parents[2]
        script = (
            "import bcrypt; "
            "from src.core.auth.service import hash_password, verify_password; "
        ...
        assert result.stdout.strip() == "5.0.0"
    ...
    def test_hash_password_preserves_legacy_72_byte_truncation(self):
        """TS-E2E-SEC-TNT-001: bcrypt 5 migration preserves legacy ASCII truncation semantics."""
        hashed = hash_password("a" * 72 + "ignored suffix")

        assert verify_password("a" * 72 + "different suffix", hashed) is True

    def test_hash_password_preserves_legacy_multibyte_truncation(self):
        """TS-E2E-SEC-TNT-001: bcrypt 5 migration truncates encoded bytes, not characters."""
        hashed = hash_password("é" * 40)

        assert verify_password("é" * 36 + "different suffix", hashed) is True

    def test_verify_password_accepts_existing_passlib_bcrypt_hash(self):
        """TS-E2E-SEC-TNT-001: bcrypt 5 verifies hashes generated by the previous Passlib adapter."""
        legacy_hash = "$2b$04$DfhNMfZlME5Du9qQTY0mcuR3Lhxgn5Vqm9rhbmmPkhGZsHsHpnoSy"

        assert verify_password("LegacyPassword123!", legacy_hash) is True

    def test_verify_password_preserves_invalid_hash_value_error(self):
        """TS-E2E-SEC-TNT-001: invalid stored hashes retain the established ValueError contract."""
