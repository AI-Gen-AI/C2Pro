...
def create_mock_request(
    path: str, method: str = "GET", headers: dict = None, query_params: dict = None
) -> Request:
    if headers is None:
        headers = {}
...
async def test_tenant_isolation_middleware_revoked_token():
    ...
    with patch(
        "src.core.middleware.tenant_isolation.is_token_revoked_async", AsyncMock(return_value=True)
    ):
        response = await middleware.dispatch(request, call_next)
...
async def test_tenant_isolation_middleware_clerk_fallback():
    ...
    user_record = MagicMock(
        user_id=internal_user_id, tenant_id=tenant_id, is_active=True, clerk_user_id=clerk_user_id
    )
    ...
    with patch(
        ...
        with patch(
            ...
            AsyncMock(return_value={"sub": clerk_user_id}),
        ):
            with patch.object(
                middleware, "_get_clerk_user_record", AsyncMock(return_value=user_record)
            ):
                with patch.object(
                    middleware, "_validate_tenant_exists", AsyncMock(return_value=True)
                ):
                    with patch("structlog.contextvars.bind_contextvars"):
                        response = await middleware.dispatch(request, call_next)
    ...
    assert response.status_code == 200
    assert request.state.tenant_id == tenant_id
    # request.state.user_id must be the internal UUID, not the Clerk "user_..." string,
    # so UUID-typed persistence (e.g. documents.created_by) does not fail on upload.
    assert request.state.user_id == internal_user_id
...
async def test_tenant_isolation_middleware_clerk_fallback_blocked():
    ...
    with patch(
        ...
        with patch(
            "src.core.middleware.tenant_isolation.verify_clerk_token",
            AsyncMock(side_effect=BootstrapFallbackBlockedError()),
        ):
            response = await middleware.dispatch(request, call_next)
...
async def test_validate_tenant_exists_not_found():
    ...
    with patch("src.core.middleware.tenant_isolation.get_raw_session", return_value=AsyncMock()):
        with patch(
            "src.core.middleware.tenant_isolation.lookup_tenant_by_id", AsyncMock(return_value=None)
        ):
            result = await middleware._validate_tenant_exists(tenant_id)
            assert result is False
...
async def test_get_tenant_for_clerk_user_exception():
    ...
    middleware = TenantIsolationMiddleware(None)
    with patch(
        "src.core.middleware.tenant_isolation.get_raw_session", side_effect=Exception("DB Error")
    ):
...
async def test_validate_tenant_exists_test_bypass():
    ...
    middleware = TenantIsolationMiddleware(None)
    with patch(
        "src.core.middleware.tenant_isolation.get_raw_session",
