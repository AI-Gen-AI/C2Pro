...
def test_dependency_audit_resolves_backend_transitive_dependencies() -> None:
    """Test Suite ID: TS-CI-BACKEND-GUARDS-001, TASK-DEV-013."""

    repo_root = Path(__file__).resolve().parents[4]
    workflow = repo_root / ".github" / "workflows" / "dependency-audit.yml"
    contents = workflow.read_text(encoding="utf-8")

    assert "pip-audit -r apps/api/requirements.txt" in contents
    assert "--disable-pip" not in contents
    assert "--no-deps" not in contents
...
def test_backend_requirements_include_langchain_anthropic_compatible_sdk() -> None:
    ...
    repo_root = Path(__file__).resolve().parents[4]
    requirements = repo_root / "apps" / "api" / "requirements.txt"
    sprint_requirements = repo_root / "apps" / "api" / "requirements-sprint1.txt"
    contents = requirements.read_text(encoding="utf-8")
    sprint_contents = sprint_requirements.read_text(encoding="utf-8")

    assert "langchain-anthropic==1.4.8" in contents
    assert "anthropic>=0.120.0,<1.0.0" in contents
    assert "pydantic-settings>=2.15.0,<3.0.0" in contents
    assert "pydantic-settings>=2.15.0,<3.0.0" in sprint_contents
    assert "supabase==2.31.0" in contents
    assert "httpx>=0.28.1,<1.0.0" in contents
    assert "uvicorn[standard]>=0.52.1,<1.0.0" in contents
    assert "uvicorn[standard]>=0.52.1,<1.0.0" in sprint_contents
...
def test_core_coherence_and_modules_jobs_are_required_gates_in_ci_workflow() -> None:
    ...
    for job_name, suite in (
        ...
        assert any(suite in step.get("run", "") for step in jobs[job_name]["steps"])
        assert job_name in jobs["ci-status"]["needs"]
