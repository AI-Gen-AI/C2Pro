...
def test_format_output_surfaces_audit_coverage_and_category_scores():
    ...
    # assessed/total may be int or float – accept both
    assert result.audit_coverage.get("assessed") == 2 or result.audit_coverage.get("assessed") == 2.0
    assert result.audit_coverage.get("total") == 6 or result.audit_coverage.get("total") == 6.0
    assert result.audit_coverage.get("pct") is not None
    ...
    # Verify honest-null preserved for unassessed
    assert result.category_scores["BUDGET"] is None
