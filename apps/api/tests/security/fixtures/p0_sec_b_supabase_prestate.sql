-- P0-SEC-B Supabase-historical pre-state fixture (PATH B).
--
-- Reproduces the state created by the June 2026 Supabase CLI migrations
-- (20260613000100_add_project_state_and_revisions.sql and
--  20260614000100_add_project_events_snapshots_artifacts.sql).
--
-- KEY DIFFERENCES from the Alembic prestate (p0_sec_b_prestate.sql):
--   * The 6 COALESCE tables have NULLIF *_tenant_isolation_* policies, NOT
--     COALESCE fail-open policies.  They are already fail-closed but under
--     Supabase-historical names.
--   * Historical UPDATE policies carry BOTH USING AND WITH CHECK (faithful to
--     what the June 2026 Supabase migrations emitted).
--   * The 4 excess-policy tables are identical to the Alembic prestate.
--
-- After P0-SEC-B upgrade (MIRROR applied via MIRROR.read_text()):
--   * The 24 *_tenant_isolation_* policies are DROPPED (IF EXISTS — they exist
--     on this path).
--   * The 24 canonical short-named NULLIF policies are CREATED.
--   * Both migration paths converge to exactly 4 canonical NULLIF policies per
--     COALESCE table with identical pg_policies catalog entries.
--
-- Test UUIDs:
...
-- ================================================================ 6 COALESCE tables
-- These tables were created by the June 2026 Supabase CLI migrations with NULLIF
-- fail-closed policies under *_tenant_isolation_* names — NOT the Alembic
-- short-named COALESCE policies.  Historical UPDATE policies include BOTH USING
-- AND WITH CHECK (the Supabase CLI emitted both clauses).

-- NULLIF fail-closed expression (already secure, just wrong policy name):
...
-- Identical to the Alembic prestate — these tables were NOT created by the June
-- 2026 Supabase migrations so their prestate is the same on both paths.
...
-- Inserted as superuser (BYPASSRLS) so they bypass the RLS policies.
