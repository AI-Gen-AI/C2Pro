...
class TestDTOSerialization:
    ...
    def test_001_to_python_keeps_uuid_in_python_mode(self) -> None:
        ...
        dto = _SampleDTO(itemId=uuid4(), status=_Status.OK, created_at=datetime.now(UTC))
    ...
    def test_006_from_python_builds_model(self) -> None:
        serializer = DTOSerializer()
        payload = {"itemId": str(uuid4()), "status": "ok", "created_at": datetime.now(UTC).isoformat()}
    ...
    def test_010_list_to_python_serializes_all_items(self) -> None:
        serializer = DTOSerializer()
        items = [
            _SampleDTO(itemId=uuid4(), status=_Status.OK, created_at=datetime.now(UTC)),
            _SampleDTO(itemId=uuid4(), status=_Status.FAIL, created_at=datetime.now(UTC)),
        ]
    ...
    def test_014_nested_model_serializes_recursively(self) -> None:
        ...
        dto = _SampleDTO(
            ...
            child=_ChildDTO(child_id=uuid4(), created_at=datetime.now(UTC)),
        )
        data = serializer.to_python(dto, json_compatible=True)
    ...
    def test_015_round_trip_json_preserves_values(self) -> None:
        serializer = DTOSerializer()
        original = _SampleDTO(itemId=uuid4(), status=_Status.OK, created_at=datetime.now(UTC))
