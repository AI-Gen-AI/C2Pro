...
class ProjectAnalysisBase(BaseModel):
    ...
    coherence_score: int | None = Field(None, ge=0, le=100, description="Coherence score from 0 to 100")
...
class ProjectAlertBase(BaseModel):
    ...
    severity: str = Field(..., description="Severity of the alert (e.g., 'critical', 'high', 'medium', 'low')")
    ...
    affected_references: dict[str, Any] | None = Field(None, description="JSON object with references to affected parts of documents")
    status: str = Field(default='open', description="Status of the alert (e.g., 'open', 'resolved')")
