...
def get_document_repository(
    ...
    """Factory para obtener el repositorio de documentos con el tenant_id del contexto."""
    return SqlAlchemyDocumentRepository(session=db_session, tenant_id=tenant_id)


def get_upload_use_case(
    storage_service: IStorageService = Depends(get_storage_service),
    document_repository: SqlAlchemyDocumentRepository = Depends(
        get_document_repository
    ),
) -> UploadDocumentUseCase:
    return UploadDocumentUseCase(
        storage_service=storage_service, document_repository=document_repository
    )
...
async def upload_document(
    ...
    document = await upload_use_case.execute(
        file=file, project_id=project_id, tenant_id=tenant_id
    )
