...
def upgrade() -> None:
    ...
    op.create_index(
        "ix_tenants_subscription", "tenants", ["subscription_plan", "subscription_status"]
    )
    ...
    op.create_table(
        ...
        sa.Column(
            "role",
            postgresql.ENUM("admin", "user", "viewer", "api", name="userrole", create_type=False),
            nullable=False,
        ),
        sa.Column("is_active", sa.Boolean(), nullable=False, server_default="true"),
    ...
    op.create_table(
        ...
        sa.Column(
            "status",
            postgresql.ENUM(
                "draft", "active", "completed", "archived", name="projectstatus", create_type=False
            ),
            nullable=False,
        ),
        ...
        sa.PrimaryKeyConstraint("id"),
        sa.CheckConstraint(
            "coherence_score >= 0 AND coherence_score <= 100", name="ck_projects_coherence_score"
        ),
    )
